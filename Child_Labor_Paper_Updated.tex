...
\begin{abstract}
\noindent This paper examines whether supply chain monitoring reduces child labor in developing countries, exploiting the 2013 Rana Plaza disaster as a quasi-experimental shock to factory oversight in Bangladesh. Using geocoded household survey data for 100,473 children linked to factory locations via cluster-level GPS coordinates, we employ a difference-in-differences strategy comparing areas within 10km of textile factories to more distant locations. We find a precisely estimated null effect on child labor ($-0.6$ pp, $p = 0.46$); equivalence testing formally rejects effects larger than $\pm 2$ pp ($p = 0.03$). The null is robust across trend-adjusted specifications, matching methods, triple-difference designs, and continuous treatment measures. However, subgroup analysis reveals meaningful heterogeneity: girls near monitored factories experience a significant $3.25$ pp increase in school enrollment ($p = 0.012$), with no corresponding effect for boys (triple interaction $p = 0.004$). Middle-wealth households show enrollment gains of $5$--$6$ pp ($p < 0.01$). Lee (2009) bounds confirm these results are robust to selective attrition. A decomposition reveals that child labor is concentrated in agriculture and the informal sector, outside the scope of factory audits. These findings demonstrate that factory monitoring alone does not reduce child labor but may generate positive educational spillovers, particularly for girls and middle-income households.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{results/new_results/event_study_replication.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/event_study_by_age.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/event_study_age10_13_ref_comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/honest_did_analysis.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/robustness_analysis.png}
...
\subsection{Alternative Outcomes and Heterogeneity}

A null finding on child labor may mask meaningful effects on related margins: school enrollment, years of education, work formality, or shifts across demographic subgroups. We systematically examine these possibilities.

\subsubsection{Multiple Outcomes}

Table \ref{tab:multiple_outcomes} presents DiD estimates for four outcomes among children aged 10--13, using the same specification as our main analysis.

\input{results/new_results/table_multiple_outcomes}

The null extends beyond child labor: school enrollment shows a positive but statistically insignificant increase ($+1.6$ pp, $p = 0.16$), and paid work shows no change. Education years show a statistically significant negative coefficient ($-0.23$ years, $p = 0.02$); however, we investigate this further below and find it reflects cohort composition rather than a causal effect.

\paragraph{Education years: sensitivity to reference wave.} The significant education years coefficient invites scrutiny. We re-estimate the DiD using each pre-period wave separately as the comparison period (Table \ref{tab:edyears_wave}).

\input{results/new_results/table_edyears_wave}

The effect is large and significant when using the earliest waves as the reference period ($-0.37$ years with 2000 or 2004 base, $p = 0.01$ and $p = 0.001$, respectively) but disappears with more proximate reference periods ($-0.11$ with 2007 base, $p = 0.42$; $-0.07$ with 2011 base, $p = 0.44$). This pattern is inconsistent with a causal effect of post-2013 monitoring---which should produce the largest contrast against the immediately preceding period---and instead indicates that early-wave cohorts differ systematically in education levels from later cohorts. The pooled estimate is significant only because the 2000 and 2004 waves, which account for nearly half of the pre-period sample, pull the average downward. We conclude that the education years finding is a composition artifact and not a treatment effect.

\subsubsection{Heterogeneity by Wealth, Gender, and Location}

The average null may mask offsetting effects across subgroups. Tables \ref{tab:wealth_hetero} and \ref{tab:demo_hetero} present heterogeneous treatment effects.

\input{results/new_results/table_wealth_heterogeneity}

By wealth quintile, the largest (though insignificant) point estimate is for the poorest households ($-3.2$ pp, $p = 0.21$), with a monotonic gradient toward positive coefficients for richer households. School enrollment effects are largest and statistically significant for middle ($+6.0$ pp, $p < 0.01$) and richer ($+4.7$ pp, $p < 0.01$) quintiles.

\input{results/new_results/table_demographic_heterogeneity}

By gender, effects diverge: a positive but insignificant coefficient for boys ($+0.8$ pp) and a negative but insignificant coefficient for girls ($-1.9$ pp, $p = 0.107$). Neither is statistically significant. Urban areas show a positive coefficient ($+2.0$ pp, insignificant), while rural areas show a near-zero effect ($+0.5$ pp). No subgroup produces a statistically significant treatment effect on child labor.

\paragraph{Gender divergence in enrollment.} While child labor effects by gender are individually insignificant, the gender pattern is strikingly different for school enrollment. Table \ref{tab:gender_divergence} reports gender-specific DiD estimates for both outcomes.

\input{results/new_results/table_gender_divergence}

Girls show a statistically significant increase in school enrollment of $3.25$ percentage points ($p = 0.012$), while the corresponding estimate for boys is exactly zero ($p = 0.999$). The triple interaction term Near $\times$ Post $\times$ Female is significant for enrollment ($+2.85$ pp, $p = 0.004$), confirming a formally significant gender differential. Figure \ref{fig:gender_es} presents gender-specific event studies.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/gender_event_study.png}
\caption{Gender-Specific Event Studies: Child Labor and School Enrollment (Ages 10--13)}
\label{fig:gender_es}
\begin{figurenotes}
Notes: Event study coefficients estimated separately by gender. Reference year is 2011. Top row: boys; bottom row: girls. Left column: child labor; right column: school enrollment. Vertical bars represent 95\% confidence intervals. Red dashed line indicates Rana Plaza disaster.
\end{figurenotes}
\end{figure}

The event studies reveal that the girls' enrollment effect is concentrated in the post-period, with pre-treatment coefficients close to zero, supporting a causal interpretation. Combined with the marginally insignificant reduction in child labor for girls ($-1.9$ pp, $p = 0.107$), this pattern is consistent with monitoring-induced shifts from work toward schooling among girls, though the child labor effect alone does not reach conventional significance levels.

\paragraph{Enrollment effects by wealth quintile.} The enrollment heterogeneity by wealth is the most robust finding in our subgroup analysis. Table \ref{tab:enrollment_wealth} reports quintile-specific effects and a pooled estimate for the middle and richer quintiles.

\input{results/new_results/table_enrollment_wealth}

Middle-wealth households show a $6.0$ pp increase in enrollment ($p = 0.005$), and richer households show a $4.7$ pp increase ($p = 0.004$). Pooling these two quintiles yields a $5.1$ pp increase ($p = 0.002$). The poorest, poorer, and richest quintiles show positive but statistically insignificant effects. Figure \ref{fig:enrollment_wealth_es} presents event study patterns by wealth group.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/enrollment_wealth_event_study.png}
\caption{School Enrollment Event Study by Wealth Group (Ages 10--13)}
\label{fig:enrollment_wealth_es}
\begin{figurenotes}
Notes: Event study coefficients for school enrollment estimated separately by wealth group. Poorest+Poorer, Middle, and Richer+Richest. Reference year is 2011. Vertical bars represent 95\% confidence intervals. Red dashed line indicates Rana Plaza disaster.
\end{figurenotes}
\end{figure}

The inverted-U pattern---with the strongest effects in the middle of the wealth distribution---is consistent with a model in which the poorest households cannot substitute schooling for child labor regardless of monitoring, and the richest already have high enrollment rates, leaving the middle quintiles most responsive to changes in labor market conditions around factories.

\subsubsection{Dose-Response Analysis}

Table \ref{tab:dose_response} exploits continuous variation in factory proximity, using three measures: proximity score ($1/(1+\text{distance})$), log distance, and count of factories within 10km.

\input{results/new_results/table_dose_response}

All three continuous treatment measures produce small and statistically insignificant coefficients, confirming the null result is not an artifact of the binary 10km threshold.

\subsubsection{Multi-Outcome Event Study}

Figure \ref{fig:multi_outcome_es} presents event study coefficients for child labor, school enrollment, and current work status, using 2011 as the reference year.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{results/new_results/multi_outcome_event_study.png}
\caption{Multi-Outcome Event Study (Ages 10--13)}
\label{fig:multi_outcome_es}
\begin{figurenotes}
Notes: Panels show event study coefficients for child labor, school enrollment, and current work status. Reference year is 2011. Vertical bars represent 95\% confidence intervals. Red dashed line indicates Rana Plaza disaster.
\end{figurenotes}
\end{figure}

The event study patterns are flat across all three outcomes, with no evidence of a structural break at the time of the Rana Plaza disaster. Pre-treatment coefficients are close to zero and statistically insignificant for all outcomes.

\subsubsection{Decomposition of Child Labor by Work Type}

If child labor is concentrated in agriculture and the informal sector, factory monitoring would not be expected to reduce it. Table \ref{tab:decomposition} examines the composition of child labor.

\input{results/new_results/table_work_decomposition}

Among working children aged 10--13 in the pre-period, approximately 41\% receive cash payment, with the remainder working without pay or in-kind only. This is consistent with predominantly informal and agricultural child labor---activities outside the scope of factory-level monitoring. The decomposition provides a structural explanation for the null finding: monitoring targets formal factory employment, but the vast majority of child labor occurs in sectors unaffected by supply chain audits.


\subsection{Brand-Specific and Multi-Brand Heterogeneity}

Our main specification pools all monitored factories regardless of which international brand or monitoring body oversees them. However, brands differ in their supply chain governance capacity, monitoring intensity, and remediation enforcement. We exploit the geocoded factory-brand mapping to test whether proximity to factories supplying specific brands generates heterogeneous treatment effects.

For each major brand, we identify the set of factories in its supply chain using the Accord/Alliance disclosure lists and the geocoded factory census. We then construct brand-specific treatment indicators---equal to one if a child resides within 10km of at least one factory supplying that brand---and estimate separate DiD regressions for ages 10--13.

\subsubsection{Brand-Specific DiD Estimates}

Table \ref{tab:brand_did} reports the results. Figure \ref{fig:brand_did} provides the corresponding visual summary.

\begin{table}[H]
\centering
\caption{Brand-Specific DiD Estimates: Child Labor (Ages 10--13)}
\label{tab:brand_did}
\begin{threeparttable}
\begin{tabular}{lccccc}
\toprule
Brand & Factories & Treated (\%) & Effect (pp) & SE & $p$-value \\
\midrule
H\&M & 21 & 7.9 & $-$2.31 & (0.72) & 0.001*** \\
Gap & 4 & 4.2 & $-$1.92 & (0.60) & 0.001*** \\
Walmart & 39 & 8.6 & $-$1.73 & (0.77) & 0.025** \\
Uniqlo & 12 & 7.5 & $-$1.28 & (0.65) & 0.047** \\
C\&A & 70 & 8.9 & $-$1.45 & (0.79) & 0.066* \\
Primark & 35 & 6.2 & $-$1.73 & (1.12) & 0.121 \\
M\&S & 13 & 4.1 & $-$1.87 & (1.95) & 0.337 \\
PVH & 106 & 9.1 & $-$0.62 & (1.46) & 0.670 \\
Mango & 79 & 10.9 & $-$0.38 & (1.13) & 0.734 \\
\midrule
All factories (baseline) & 774 & 8.7 & $-$0.56 & (0.75) & 0.460 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Notes: Each row is a separate DiD regression with the brand-specific within-10km indicator interacted with post-Rana Plaza. Controls: age dummies, sex. Standard errors clustered at district level. $N = 32{,}017$ for all regressions. *** $p<0.01$, ** $p<0.05$, * $p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{results/new_results/brand_specific_did.png}
\caption{Brand-Specific DiD Estimates (Ages 10--13)}
\label{fig:brand_did}
\begin{figurenotes}
Notes: Point estimates and 95\% confidence intervals for the Near Brand Factory $\times$ Post interaction. Dashed line shows the pooled all-factory estimate ($-$0.66 pp). Number of factory locations in parentheses. Standard errors clustered at district level.
\end{figurenotes}
\end{figure}

Several brand-specific estimates are individually significant: H\&M ($-$2.31 pp, $p = 0.001$), Gap ($-$1.92 pp, $p = 0.001$), Walmart ($-$1.73 pp, $p = 0.025$), and Uniqlo ($-$1.28 pp, $p = 0.047$). However, the brands with the most factory locations---PVH (106 locations) and Mango (79 locations)---show small and insignificant effects. This pattern suggests that the significant brand-specific estimates may reflect geographic concentration in specific areas (e.g., the Dhaka--Chittagong corridor) rather than genuine brand-level monitoring effects. Brands with fewer, more spatially concentrated factories mechanically produce treatment indicators that are more correlated with specific local labor market conditions.

\subsubsection{Brand-Tier Aggregation}

To increase statistical power and reduce the influence of idiosyncratic geographic clustering, we aggregate brands into tiers based on market positioning and group their factory networks:

\begin{table}[H]
\centering
\caption{Brand-Tier DiD Estimates: Child Labor (Ages 10--13)}
\label{tab:brand_tier}
\begin{threeparttable}
\begin{tabular}{llcccc}
\toprule
Tier & Brands & Factories & Effect (pp) & SE & $p$-value \\
\midrule
Premium fast fashion & H\&M, Uniqlo, Mango & 108 & $-$0.40 & (1.12) & 0.719 \\
Value fast fashion & Primark, C\&A & 93 & $-$1.58 & (0.83) & 0.056* \\
Mass market & Walmart, Gap & 42 & $-$1.65 & (0.74) & 0.026** \\
European premium & M\&S, PVH & 109 & $-$1.04 & (1.12) & 0.349 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Notes: Each row is a separate DiD regression using the combined factory network of all brands in the tier. Treatment: within 10km of any factory in the tier. Controls: age dummies, sex. SEs clustered at district level. $N = 32{,}017$. *** $p<0.01$, ** $p<0.05$, * $p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}

The tier-level results are revealing. Premium fast fashion brands (H\&M, Uniqlo, Mango), which individually showed mixed results, produce a null aggregate effect ($-$0.40 pp, $p = 0.72$) once their factory networks are combined. This is consistent with the interpretation that individual brand significance reflects geographic idiosyncrasies rather than brand-level treatment heterogeneity. The mass market tier (Walmart, Gap) shows a marginally significant negative effect ($-$1.65 pp, $p = 0.03$), though this is driven by a small number of factory locations concentrated in specific districts.

\subsubsection{Accord versus Alliance}

We separately estimate effects for factories monitored under the Accord on Fire and Building Safety versus the Alliance for Bangladesh Worker Safety. These organizations differed in governance structure (the Accord was union-backed with legally binding commitments; the Alliance was industry-led with voluntary participation) and in their approach to remediation enforcement.

\begin{table}[H]
\centering
\caption{Accord vs.\ Alliance DiD Estimates: Child Labor (Ages 10--13)}
\label{tab:accord_alliance}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
Organization & Factory Locations & Effect (pp) & SE & $p$-value \\
\midrule
Accord & 116 & $+$0.02 & (1.32) & 0.990 \\
Alliance & 48 & $-$1.26 & (1.00) & 0.206 \\
All factories (baseline) & 774 & $-$0.56 & (0.75) & 0.460 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Notes: Treatment defined as within 10km of a factory with GPS coordinates in the respective organization's database. The Accord and Alliance factory lists overlap partially. Controls: age dummies, sex. SEs clustered at district level. $N = 32{,}017$. *** $p<0.01$, ** $p<0.05$, * $p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}

Neither organization produces a significant effect on child labor. The Accord estimate is essentially zero ($+$0.02 pp, $p = 0.99$), while the Alliance estimate is negative but insignificant ($-$1.26 pp, $p = 0.21$). The difference between the two is not statistically significant, providing no evidence that the stronger enforcement mechanisms of the Accord translated into greater reductions in child labor relative to the Alliance's voluntary framework.

Taken together, the brand-level and organization-level analyses reinforce our main finding: the null treatment effect is not an artifact of pooling heterogeneous brand effects. While some individual brand estimates are statistically significant, these results do not survive aggregation to the tier level, consistent with geographic confounding rather than genuine brand-specific monitoring effects.


\subsection{Placebo Test: Health Facility Quality}

As an additional falsification exercise, we test whether proximity to garment factories predicts the quality of health facilities---an outcome that factory monitoring programs should not affect. Using the 2014 Bangladesh Service Provision Assessment (SPA), which surveyed 1,587 health facilities with geocoded locations, we compute the distance from each facility to the nearest garment factory and regress facility characteristics on a within-10km indicator, controlling for region, facility type, and managing authority fixed effects.

Table \ref{tab:spa_placebo} presents the results. Of 19 outcomes tested, 6 are nominally significant at the 5\% level, and 4 survive Benjamini-Hochberg correction for multiple testing. The significant coefficients---on electricity access, number of providers, immunization services, and service breadth---reflect the geographic correlation between factory clusters and urban infrastructure rather than any causal effect of factory monitoring on health services. Factories concentrate in and around Dhaka and Chittagong, where health facilities are better resourced regardless of monitoring interventions. Critically, composite indices for infrastructure, equipment, and child health services show no significant differences after conditioning on fixed effects ($p = 0.65$, $p = 0.27$, $p = 0.16$, respectively).

These results support the interpretation that our null child labor finding reflects the absence of a genuine treatment effect rather than insufficient statistical power: the treatment variable successfully captures geographic variation in factory proximity, but this variation does not predict outcomes in sectors unrelated to garment manufacturing.

\begin{table}[H]
\centering
\caption{Placebo Test: Factory Proximity and Health Facility Quality (2014 SPA)}
\label{tab:spa_placebo}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
Outcome & Coef. & SE & $p$-value & $N$ \\
\midrule
\multicolumn{5}{l}{\textit{Panel A: Composite Indices}} \\
Infrastructure index (0--6) & 0.085 & (0.190) & 0.653 & 1,548 \\
Equipment index (0--8) & 0.207 & (0.189) & 0.272 & 1,548 \\
Service breadth index (0--8) & 0.470 & (0.179) & 0.008*** & 1,548 \\
Child health index (0--5) & 0.228 & (0.163) & 0.163 & 1,463 \\
Cleanliness score (0--8) & 0.531 & (0.235) & 0.024** & 1,548 \\
\\
\multicolumn{5}{l}{\textit{Panel B: Infrastructure and Staffing}} \\
Connected to electricity & 0.190 & (0.058) & 0.001*** & 1,548 \\
Number of providers & 1.871 & (0.656) & 0.004*** & 1,548 \\
Total staff & 24.920 & (16.105) & 0.122 & 1,548 \\
Number of beds & 7.703 & (4.364) & 0.078* & 1,534 \\
\\
\multicolumn{5}{l}{\textit{Panel C: Service Availability}} \\
Immunization services & 0.159 & (0.046) & 0.001*** & 1,548 \\
Sick child services & 0.004 & (0.037) & 0.908 & 1,548 \\
Antenatal care & $-$0.049 & (0.031) & 0.107 & 1,548 \\
Delivery services & 0.096 & (0.042) & 0.023** & 1,548 \\
\\
\multicolumn{5}{l}{\textit{Panel D: Equipment and Hygiene}} \\
Soap in OPD & $-$0.067 & (0.053) & 0.201 & 1,548 \\
Visual privacy & 0.046 & (0.053) & 0.384 & 1,548 \\
Water onsite & $-$0.025 & (0.034) & 0.449 & 1,430 \\
Functioning computer & $-$0.080 & (0.084) & 0.341 & 1,548 \\
Outpatient visits (monthly) & $-$48.625 & (258.023) & 0.851 & 1,479 \\
Any client fees & 0.084 & (0.044) & 0.055* & 1,548 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Notes: OLS regressions of facility outcomes on a within-10km-of-factory indicator. Controls: region FE, facility type FE, managing authority FE. Robust (HC1) standard errors. Sample: 1,587 health facilities from the 2014 Bangladesh SPA (113 near factory, 1,474 far). BH-adjusted: 4 of 19 significant at $q < 0.05$. *** $p<0.01$, ** $p<0.05$, * $p<0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\subsection{Enrollment Spillovers: Mechanisms}

While the aggregate null on child labor is robust, the significant enrollment gains for girls ($+3.25$ pp, $p = 0.012$) and middle-wealth households ($+5$--$6$ pp, $p < 0.01$) require a coherent explanation. We argue these findings are predicted by the gender composition of Bangladesh's garment workforce and the economics of household educational investment.

Bangladesh's ready-made garment (RMG) sector employs approximately 4 million workers, roughly 80\% of whom are women \citep{HeathMobarak2015}. \citet{HeathMobarak2015} demonstrated that garment sector \textit{expansion} increased girls' school enrollment and delayed marriage: exposure to factory jobs raised the perceived returns to female education by creating viable formal-sector employment opportunities that reward basic literacy and numeracy. Our finding extends this logic from sector expansion to sector \textit{formalization}. Post-Rana Plaza monitoring---with its highly visible international inspections, publicized compliance reports, and factory remediation programs---signaled that garment employment would not only continue but become safer and more regulated. For households near monitored factories, this reinforced the expectation that formal-sector jobs requiring education would remain available for daughters. Boys do not respond because garment employment is not their expected career path: male labor force participation in Bangladesh is concentrated in agriculture, construction, and transport, sectors unaffected by supply chain monitoring.

The wealth heterogeneity follows naturally. The poorest households face binding liquidity constraints: even if monitoring raises perceived returns to girls' education, these families cannot afford school fees, uniforms, or the opportunity cost of forgone earnings. The richest households already have near-universal enrollment, leaving little room for improvement. Middle-wealth households---the ``Richer'' and ``Middle'' quintiles in our classification---are on the margin of the schooling decision. For these families, the signal of continued formal-sector opportunity is sufficient to tip the cost-benefit calculation toward enrollment. This produces the inverted-U pattern we observe, with the strongest effects concentrated in the middle of the wealth distribution.

Two caveats temper this interpretation. First, these are subgroup findings, and while the gender differential is formally significant (triple interaction $p = 0.004$), multiple testing across subgroups raises the probability of false discovery. We note, however, that the gender specificity is \textit{predicted ex ante} by the female composition of the garment workforce, rather than a post hoc rationalization. Second, we cannot directly observe parental expectations or aspirations in the DHS data. The mechanism linking monitoring to perceived returns to education remains inferential, though it is grounded in the empirical findings of \citet{HeathMobarak2015} in the same institutional context.

\subsection{Precision of the Null Result}

A common concern with null findings is that the study may simply be underpowered. We address this through two complementary approaches: minimum detectable effect (MDE) analysis and formal equivalence testing.

\subsubsection{Power Analysis and Minimum Detectable Effect}

With a standard error of 0.75 percentage points, our study can detect effects of 2.1 pp or larger at 80\% power (two-sided, $\alpha = 0.05$). This MDE represents 26\% of the baseline child labor rate (8.0\%) among children aged 10--13. Table \ref{tab:power_analysis} compares this MDE to benchmark effects from the child labor literature.

\input{results/new_results/table_power_analysis}

Our study is well-powered to detect effects comparable to those found in major child labor interventions: conditional cash transfer programs (5--10 pp; \citealp{dehoop2014cash}), trade liberalization shocks (3--6 pp; \citealp{edmonds2005does, edmonds2010trade}), and school construction programs (3--5 pp; \citealp{ravallion2000wealth}). We can rule out effects of this magnitude with high confidence. Figure \ref{fig:power_curve} plots the full power curve.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{results/new_results/power_curve.png}
\caption{Power Curve: Detectable Effects on Child Labor}
\label{fig:power_curve}
\begin{figurenotes}
Notes: Power curve for DiD estimate of factory monitoring on child labor, ages 10--13. Horizontal lines mark 80\% and 90\% power thresholds. Vertical green line indicates baseline child labor rate. MDE at 80\% power is 2.1 pp.
\end{figurenotes}
\end{figure}

\subsubsection{Equivalence Testing}

Standard hypothesis testing cannot distinguish between ``no effect'' and ``underpowered study.'' We implement the Two One-Sided Tests (TOST) procedure to formally test and \textit{reject} the hypothesis that effects exceed a policy-relevant threshold \citep{lakens2017equivalence}.

The TOST procedure tests $H_0: |\beta| \geq \delta$ against $H_1: |\beta| < \delta$ by conducting two one-sided tests. Rejection of $H_0$ at significance level $\alpha$ provides positive evidence that the true effect lies within the equivalence bounds $(-\delta, +\delta)$.

Table \ref{tab:tost} reports results for several equivalence bounds.

\input{results/new_results/table_tost}

We reject effects larger than $\pm 2$ pp at the 5\% level ($p_{\text{TOST}} = 0.03$), with the smallest rejectable bound at $\pm 1.9$ pp. In other words, we can conclude with 95\% confidence that the true effect of factory monitoring on child labor is smaller than 1.9 percentage points in absolute value. This formally establishes our finding as a precisely estimated null rather than an underpowered study.

\subsubsection{Lee Bounds for Selective Attrition}

A remaining concern is whether differential attrition across treatment and control groups could bias our estimates. We implement the \citet{lee2009training} trimming bounds procedure, which provides worst-case bounds on the treatment effect under selective sample attrition. Table \ref{tab:lee_bounds} reports results for the full sample and for subgroups with notable point estimates.

\input{results/new_results/table_lee_bounds}

Differential attrition is negligible in all subgroups: trimming fractions are effectively zero, and selection tests are insignificant for the enrollment outcomes ($p > 0.6$). Consequently, Lee bounds coincide with the point estimates. This confirms that our findings---including the girls' enrollment effect and the middle/richer enrollment gains---are not driven by selective sample composition.
...
This paper provides a precisely estimated null effect of supply chain monitoring on child labor following the 2013 Rana Plaza disaster in Bangladesh. Using cluster-level GPS coordinates from DHS surveys linked to geocoded factory locations, we find no evidence that post-Rana Plaza factory monitoring reduced child labor in surrounding communities.

Our analysis yields five main findings. First, the treatment effect on child labor is small and precisely estimated. The point estimate for children aged 10--13 is $-0.56$ percentage points ($p = 0.46$). Using equivalence testing (TOST), we formally reject effects larger than $\pm 2$ pp ($p = 0.03$), ruling out effect sizes comparable to conditional cash transfer programs, trade liberalization shocks, and school construction programs documented in the literature.

Second, the parallel trends assumption is well-supported. The estimated pre-trend is negligible ($+0.02$ pp/year, $p = 0.93$), and treatment effects are stable across all trend-adjusted specifications, matching methods, triple-difference designs, and the \citet{RambachanRoth2023} sensitivity framework.

Third, while the aggregate null holds for child labor, subgroup analysis reveals meaningful heterogeneity in school enrollment. Girls near monitored factories experience a significant $3.25$ pp increase in enrollment ($p = 0.012$), with a formally significant gender differential (triple interaction $p = 0.004$). Middle-wealth and richer households show enrollment gains of $5$--$6$ pp ($p < 0.01$), while the poorest and richest quintiles show no significant response. Lee (2009) bounds confirm these findings are robust to selective attrition. The inverted-U pattern across wealth quintiles is consistent with a model in which the poorest households cannot substitute schooling for child labor and the richest already have high enrollment, leaving the middle of the distribution most responsive.

Fourth, the significant education years coefficient ($-0.23$ years, $p = 0.02$) is a composition artifact: it is driven entirely by early survey waves (2000, 2004) and disappears when using 2007 or 2011 as the reference period ($p > 0.4$), ruling out a causal interpretation.

Fifth, a decomposition of child labor reveals that working children are concentrated in agriculture and the informal sector---with approximately 59\% of working children receiving no cash payment. This provides a structural explanation: factory monitoring targets formal-sector compliance, but child labor in Bangladesh occurs predominantly in sectors outside the scope of supply chain audits.

These findings contribute to debates about private governance effectiveness in global supply chains \citep{Locke2007, DistelhorstLocke2018}. While the Accord and Alliance achieved meaningful improvements in factory safety, our results demonstrate that factory-level monitoring alone is insufficient to reduce child labor in surrounding communities. However, the enrollment gains for girls and middle-wealth households suggest that monitoring may generate positive educational spillovers through indirect channels---potentially by increasing perceived returns to education in areas with expanding formal-sector employment opportunities.

Future research should investigate these enrollment effects using factory-level data where monitoring is directly enforced, examine whether the gender-specific enrollment response translates to longer-run human capital accumulation, and explore complementary policies (education subsidies, conditional transfers) that may more effectively address child labor alongside supply chain monitoring.
...
\newblock \textit{Social Psychological and Personality Science}, 8(4):355--362.

\bibitem[Lee(2009)]{lee2009training}
Lee, D.S. (2009).
\newblock Training, Wages, and Sample Selection: Estimating Sharp Bounds on Treatment Effects.
\newblock \textit{Review of Economic Studies}, 76(3):1071--1102.
